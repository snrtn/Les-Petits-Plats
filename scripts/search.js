...
const start = (data, result) => {
  ...
  arrResult.push(
    ..._filter(data.recipes, value => value.ingredients.some(el => el.ingredient.toLowerCase().includes(result))),
  );
  arrResult.push(..._filter(data.recipes, value => value.appliance.toLowerCase().includes(result)));
  arrResult.push(..._filter(data.recipes, value => value.ustensils.some(el => el.toLowerCase().includes(result))));

  let reData = arrResult.filter(
